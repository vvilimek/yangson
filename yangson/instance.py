...
Instance route: A route into an instance value that is also used as the cooked
value of the instance-identifier type.
...
import json
from typing import Mapping, Optional, TYPE_CHECKING, Union, Protocol
...
class InstanceNode:
    ...
    def _merge_list(self, value: ArrayValue) -> "InstanceNode":
    ...
    def _merge_container(self, value: ObjectValue) -> "InstanceNode":
        inst = self._copy(self.value)
        rest = []
        for k in value:
            if k in self.value:
    ...
    def to_xml(self, filter: OutputFilter = OutputFilter(),
               elem: Optional[ET.Element] = None) -> ET.Element:
    ...
    def _descendants(self, qname: Union[QualName, bool, None] = None,
...
class RootNode(InstanceNode):
    ...
    def _copy(self, newval: Value, newts: Optional[datetime] = None) -> InstanceNode:
        return RootNode(newval, self.schema_node, self.schema_data,
                        newts if newts else newval.timestamp)

    def _ancestors_or_self(self,
            qname: Union[QualName, bool, None] = None) -> list["RootNode"]:
...
class ObjectMember(InstanceNode):
    ...
    def _copy(self, newval: Value, newts: Optional[datetime] = None) -> "ObjectMember":
        if newts:
            ts = newts
    ...
    def _instance_route_entry(self) -> "MemberName":
        p, s, loc = self._key.partition(":")
...
class ArrayEntry(InstanceNode):
    ...
    def _instance_route_entry(self) -> "EntryKeys":
        sn = self.schema_node
...
class InstanceRoute(tuple[InstanceRouteItem, ...]):
    ...
    def as_schema_route(self) -> "SchemaRoute":
        """Get a SchemaRoute from InstanceRoute.
        This ttransformation removes all key values and leaf-list/list indices."""
        sch_route = []
        for mem_key in self:
            if isinstance(mem_key, (EntryKeys, EntryIndex, EntryValue)):
                continue
